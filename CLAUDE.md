...
cl-mcp is a monorepo that provides an MCP (Model Context Protocol) server for component library metadata. It has four runtime packages (plus a docs site):
...
- **`@cl-mcp/docs`** (private, not published) — Documentation site built with Astro Starlight (`packages/docs`). Prose lives in `src/content/docs/*.md(x)`; the **API Reference is auto-generated at build time** by `starlight-typedoc` from the `@cl-mcp/core` + `@cl-mcp/analyzer` public entry points (config in `astro.config.mjs`, TypeDoc tsconfig in `tsconfig.typedoc.json`) into the git-ignored `src/content/docs/api/`. Build with `npm run docs:build` (builds the TS packages first so TypeDoc can resolve cross-package `@cl-mcp/*` type imports); dev with `npm run docs:dev`. Deployed to GitHub Pages (base `/cl-mcp`) by `.github/workflows/docs.yml` on push to `main`. The whole Astro stack is under `devDependencies` and `packages/docs` is excluded from Biome. When you change a package's public API or the tool set, the API Reference regenerates automatically — keep the hand-written prose pages in sync.
...
- **`@angular/compiler` is an OPTIONAL peer dependency** of the analyzer (devDependency for build/tests). It is loaded via a guarded top-level `await import()` in `src/shared/template-parser.ts` — the ONLY module allowed to load it; all value access goes through `requireAngularCompiler()` (throws `AngularCompilerUnavailableError`) / `isAngularCompilerAvailable()`, everything else imports compiler types with `import type` only. When absent: importing any package still works, `TemplateValidator.validate()` returns an `angular-compiler-unavailable` error, and `AngularFrameworkAnalyzer.analyze()` fails fast with an install hint. Don't add static value imports of `@angular/compiler` anywhere.
